...
/***************************************
	...
	作用：将插入结构体功能封装为函数
***************************************/
struct st_t *insert(int size, int *count, struct st_t stu, struct st_t *prev)
{
	struct st_t *new = NULL;
	
	new = (struct st_t *)malloc(sizeof(struct st_t) * (*count + 1));
	if (NULL == new)
	{
	...
	memcpy(new, prev, sizeof(struct st_t) * (*count));
	memcpy(new + (*count), &stu, sizeof(struct st_t));
...
void *insert_void(int size, int *count, void *stu, void *prev)
{
	void *new = NULL;
	...
	memcpy(new + size * (*count), stu, size);
 ...
 */
	...
	#endif
	#if 1
		/*new = insert(sizeof(struct st_t), &count, stu, new);*/
		newi = (int *)insert_void(sizeof(int), &count, &num, newi);
	...
	#if INTP
	for (i = 0; i < count; i++)	//逐个打印动态内存中的内容
	{
		printf("%d\n", newi[i]);
	}
	#elif
	...
	{
		printf("stu[%d] : name = %-8s id = %-8d age = %-6d sex = %c cn = %-6.1f en = %-6.1f\n", i+1, new[i].name, new[i].id, new[i].age, new[i].sex, new[i].cn, new[i].en);
	}
	#endif
