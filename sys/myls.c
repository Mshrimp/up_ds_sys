...
void month_day_time_sys(void);

void user_print(struct stat *s);
 ...
 *    tmp = time(&t);
 *    tim = localtime(&t);
 *    printf("%d-%d %d:%d", tim->tm_mon + 1, tim->tm_mday, tim->tm_hour, tim->tm_min);
 */

	strftime(buf, sizeof(buf), "%m-%d %H:%M", tim);
	printf("%s ", buf);
